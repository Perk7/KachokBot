...
def progress_menu(msg: telebot.types.Message, bot: telebot.TeleBot):
    ...
    message = bot.send_message(msg.chat.id, text='Выбери упражнение, прогресс по которому ты хочешь посмотреть' if exercs else 'У тебя нет упражнений, чтобы смотреть по ним прогресс', reply_markup=markup)
